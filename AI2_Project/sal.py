...
for x in range(1000):
	...
	# make test players	
	thisPlayer = P.Player("Davis", PT.Owl(1,0.01))
	player_list.append(thisPlayer)
	thisPlayer = P.Player("Marl", PT.Dolphin(1,0.01,0.98,0.005,"Marl"))
	player_list.append(thisPlayer)
	# thisPlayer = P.Player("Sven", PT.Salamander())
	# player_list.append(thisPlayer)	
	# thisPlayer = P.Player("Jorge", PT.Salamander())
	# player_list.append(thisPlayer)

	# make terminator ai
	thisPlayer = P.Player("Terminator", PT.Lynx(0.08,0.98,0.005,"Terminator"))
	player_list.append(thisPlayer)
